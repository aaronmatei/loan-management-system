// Phase B — the welfare member self-service read API (/api/portal/member/*).
...
async function makeMember(admin, welfareId, over = {}) {
  return (
    await request(app)
      ...
      .send({ first_name: "Jane", last_name: "Doe", phone_number: over.phone || "0716000111", id_number: over.id || "MBR1", ...over })
  ).body.data;
}
...
describe("member portal read API", () => {
  it("returns the member's own overview, ledger, loans", async () => {
    ...
    const m = await makeMember(admin, welfare.id, { phone: "0716000111", id: "MBRA1" });
    ...
    const tok = customerToken(await pcIdByPhone("+254716000111"), tenant.id);

    const ov = await request(app).get("/api/portal/member/overview").set("Authorization", tok);
    ...
    const led = await request(app).get("/api/portal/member/ledger").set("Authorization", tok);
  ...
  it("isolates members — one member can't see another's loans", async () => {
    const { tenant, admin, welfare } = await welfareSetup();
    const a = await makeMember(admin, welfare.id, { phone: "0720000001", id: "ISOA" });
    const b = await makeMember(admin, welfare.id, { phone: "0720000002", id: "ISOB" });
    ...
    const tokB = customerToken(await pcIdByPhone("+254720000002"), tenant.id);
    const loansB = await request(app).get("/api/portal/member/loans").set("Authorization", tokB);
    expect(loansB.body.data).toHaveLength(0);
    const ovB = await request(app).get("/api/portal/member/overview").set("Authorization", tokB);
  ...
  it("403s a borrower (non-member) hitting member routes", async () => {
    ...
    const m = await makeMember(admin, welfare.id, { phone: "0733000111", id: "BRW1" });
    await invite(admin, welfare.id, m.id);
    const pcId = await pcIdByPhone("+254733000111");
    ...
    const res = await request(app).get("/api/portal/member/overview").set("Authorization", tok);

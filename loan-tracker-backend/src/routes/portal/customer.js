    ...
    const r = await query(
      `SELECT
       ...
       WHERE t.status = 'active'
         ...
         AND (t.kind <> 'welfare' OR (COALESCE(t.lends_to_non_members, false) = true AND COALESCE((SELECT ws.loans_enabled FROM welfare_settings ws WHERE ws.tenant_id = t.id), false) = true))
  ...
  try {
    ...
    const r = await query(
      `SELECT
         ...
         -- Per-lender penalty interest on overdue balances (monthly %),
         -- configurable in Loan Settings; falls back to the 5% default.
         COALESCE(t.penalty_rate, 5.00)           AS penalty_rate,
         COALESCE(t.allow_self_signup, false)     AS can_self_signup
       FROM tenants t
       LEFT JOIN pawn_settings ps ON ps.tenant_id = t.id

...
// Public PAWNBROKER signup. A pawnbroker lends cash against pledged items.
// Creates a tenant of kind='pawnbroker' + admin user + capital pool + a Main
// branch + company settings, in one transaction. They get a pawn-only
// experience (their own clients + pawns); pawn interest/fees are billed by the
// platform on the standard lender basis (5% of interest), so no special
// billing is needed. Not related to other tenants.
router.post("/pawnbroker-signup", async (req, res) => {
  const {
    business_name,
    subdomain,
    contact_name,
  ...
  if (!business_name || !subdomain || !contact_name || !contact_email || !admin_password) {
    return res.status(400).json({ error: "All required fields must be provided" });
  }
  if (!validateEmail(contact_email)) {
    return res.status(400).json({ error: "Invalid email format" });
  ...
  try {
    ...
    const tRes = await client.query(
      `INSERT INTO tenants (
        ...
        registration_number, contact_name, contact_email, contact_phone,
        city, county, plan, status
      ) VALUES ($1,$2,'pawnbroker','pawnbroker',$3,$4,$5,$6,$7,$8,$9,'trial','active')
      RETURNING *`,
      [
        tenantCode,
        business_name,
    ...
    await client.query(
      `INSERT INTO branches (tenant_id, name, is_default, active) VALUES ($1, 'Main', TRUE, TRUE)`,
      [tenant.id],
    );
    ...
    // Pawnbrokers advance cash, so they need a capital pool (pawn disbursement
    // draws it down, redemption restores it — same as a normal loan).
    await client.query(
    ...
    await client.query(
      `INSERT INTO company_settings (tenant_id, company_name, company_phone, company_email)
       VALUES ($1, $2, $3, $4)`,
      [tenant.id, business_name, contact_phone || "", contact_email.toLowerCase()],
    );
    ...
    logger.info(`💎 New pawnbroker signed up: ${business_name} (${sub})`);

    res.status(201).json({
      success: true,
      message: "Welcome! Your pawnshop account is ready.",
      token,
      user: {
        ...user,
        full_name: `${user.first_name} ${user.last_name}`,
    ...
    await client.query("ROLLBACK").catch(() => {});
    logger.error("Pawnbroker signup error:", error);
    res.status(500).json({ error: "Failed to create pawnbroker account" });
  } finally {
    client.release();
  }
});

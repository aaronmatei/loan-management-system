  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-slate-100 p-6 max-w-2xl mt-6">
      <div className="flex items-start justify-between gap-3 mb-1">
        <div>
          <h2 className="font-bold text-slate-900">Loan products</h2>
          <p className="text-sm text-slate-500">Preset terms members borrow on — rate, method, fees, penalties and the amount/duration range.</p>
        </div>
      ...
      {loading ? <p className="text-sm text-slate-500 mt-4">Loading…</p> : rows.length === 0 ? (
        <p className="text-sm text-slate-400 mt-4">No loan products yet. Add one so members can apply on set terms.</p>
      ) : (
              ...
              {p.active && (
                <PermissionGate role={["admin", "manager"]}>
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => setEditing(p)} className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded" title="Edit"><Pencil size={15} /></button>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-10" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h3 className="text-lg font-bold text-slate-900">{product ? "Edit loan product" : "New loan product"}</h3>

    ...
    <div className="bg-white rounded-xl shadow-md border border-indigo-100 mb-6 overflow-hidden">
      ...
      <div className="p-5">
        {loading ? (
          ...
          <p className="text-sm text-slate-500">No messages yet. Broadcasts, reminders and receipts appear here.</p>
        ) : (
              ...
              <tbody>
                {rows.map((t) => (
                  <tr key={t.id} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2 text-slate-700">{t.phone_number}</td>
                    <td className="px-3 py-2 text-slate-600">{TYPE[t.message_type] || t.message_type}</td>
                    <td className="px-3 py-2 text-slate-600 max-w-xs truncate" title={t.message}>{t.message}</td>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md my-10" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h3 className="text-lg font-bold text-slate-900">Broadcast to members</h3>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          ...
          <p className="text-xs text-slate-500">Goes to every active member with a phone number, prefixed with the chama name.</p>
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4} maxLength={300} placeholder="e.g. Reminder: meeting this Saturday at 2pm." className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none" autoFocus />
          <p className="text-xs text-slate-400 text-right">{message.length}/300</p>

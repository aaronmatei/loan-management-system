    ...
    <div className="bg-white rounded-xl shadow-md border border-amber-100 mb-6 overflow-hidden">
      ...
      <div className="p-5">
        {loading ? (
          ...
          <p className="text-sm text-slate-500">No loans from the pool yet.</p>
        ) : (
  ...
  return (
    <ModalShell title="Issue Loan from Pool" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <p className="text-sm text-slate-600">Pool balance: <strong>{money(poolBalance)}</strong></p>
        ...
        {principal > 0 && (
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm space-y-1">
            <div className="flex justify-between"><span className="text-gray-600">Interest (flat)</span><span className="font-semibold">{money(interest)}</span></div>
            <div className="flex justify-between border-t border-slate-200 pt-1"><span className="font-bold text-gray-800">Total repayable</span><span className="font-bold text-amber-700">{money(totalDue)}</span></div>
          </div>
        )}
...
function RepayModal({ base, loan, onClose, onDone }) {
  ...
  return (
    <ModalShell title={`Repay ${loan.loan_code}`} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <p className="text-sm text-slate-600">Outstanding: <strong>{money(outstanding)}</strong>. Repayments go back into the pool.</p>
        {error && <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-lg text-sm flex items-center gap-2"><AlertTriangle size={15} /> {error}</div>}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Amount <span className="text-gray-500 font-normal">(blank = full {money(outstanding)})</span></label>

...
export default function WelfareDocumentsPanel({ client, path, admin = false }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <form onSubmit={submit} className="bg-white rounded-xl shadow-md border border-slate-100 p-5">
        <h2 className="font-bold text-slate-900 mb-1 flex items-center gap-2"><Upload size={18} className="text-emerald-600" /> Share a document</h2>
        <p className="text-sm text-slate-500 mb-4">Meeting minutes, account statements, the constitution — PDF, Office, image or text, up to 15 MB.</p>
        <div className="grid sm:grid-cols-2 gap-3">
          <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Title (e.g. March AGM minutes)" className="px-3 py-2 rounded-lg border border-slate-200 sm:col-span-2" />
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="px-3 py-2 rounded-lg border border-slate-200">
            {CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
          </select>
          {isOfficer && (
            <select value={form.visibility} onChange={(e) => setForm({ ...form, visibility: e.target.value })} className="px-3 py-2 rounded-lg border border-slate-200">
              <option value="members">Visible to all members</option>
              <option value="officers">Officers only</option>
            </select>
          )}
          <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} className="text-sm text-slate-600 sm:col-span-2 file:mr-3 file:px-3 file:py-2 file:rounded-lg file:border-0 file:bg-emerald-50 file:text-emerald-700 file:font-semibold" />
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md border border-slate-100">
        <div className="px-5 py-3 border-b border-slate-100"><h2 className="font-bold text-slate-900">Documents</h2></div>
        {loading ? <div className="p-8"><Spinner centered label="Loading documents…" /></div>
          : error ? <p className="px-5 py-8 text-center text-rose-600">{error}</p>
          : docs.length === 0 ? <p className="px-5 py-8 text-center text-slate-500">No documents yet.</p>
          : (
              ...
              {docs.map((d) => (
                <li key={d.id} className="px-5 py-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-slate-800 truncate">{d.title}
                    ...
                    <p className="text-xs text-slate-500">{CAT_LABEL[d.category] || d.category} · {d.uploaded_by_name || "—"} · {fmtDate(d.created_at)}{d.size_bytes ? ` · ${fmtSize(d.size_bytes)}` : ""}</p>
                  </div>
                  ...
                  {canDelete(d) && <button onClick={() => del(d)} className="text-slate-400 hover:text-rose-600 shrink-0" title="Delete"><Trash2 size={16} /></button>}
                </li>
              ))}
            </ul>
          )}

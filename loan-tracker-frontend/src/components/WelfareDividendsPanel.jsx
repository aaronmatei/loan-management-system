          ...
          <>
            <div className="grid grid-cols-3 gap-3 mb-5 text-sm">
              <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3">
                <p className="text-xs text-slate-500 dark:text-slate-400">Pool balance</p>
                <p className="font-bold text-slate-900 dark:text-slate-100">{money(info?.pool)}</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3">
                <p className="text-xs text-slate-500 dark:text-slate-400">Members' savings</p>
                <p className="font-bold text-slate-900 dark:text-slate-100">{money(info?.total_savings)}</p>
              </div>
            ...
            {surplus <= 0 && <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">Surplus is the pool above members' savings (penalty + loan-interest income). Settle outstanding loans to free it up.</p>}

            {history.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">No share-outs yet.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {history.map((h) => (
                      <tr key={h.id} className="border-t border-slate-100 dark:border-slate-700 hover:bg-amber-50/50 cursor-pointer" onClick={() => setOpenId(h.id)}>
                        <td className="px-4 py-2 text-slate-600 dark:text-slate-400">{fmt(h.created_at)}</td>
  ...
  return (
    <Shell title="Run share-out" onClose={onClose} wide>
      <form onSubmit={submit} className="space-y-4">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">Distributable surplus: <strong>{money(info?.surplus)}</strong></p>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-slate-200 mb-1">Basis</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-slate-200 mb-1">Amount <span className="text-gray-500 dark:text-slate-400 font-normal">(blank = full surplus)</span></label>
            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder={String(info?.surplus ?? "")} className={fld} />
          </div>
        </div>

        {preview?.preview?.length > 0 && (
          <div className="border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden max-h-56 overflow-y-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {preview.preview.map((p) => (
                  <tr key={p.member_id} className="border-t border-slate-100 dark:border-slate-700">
                    <td className="px-3 py-2 text-slate-800 dark:text-slate-100">{p.name}</td>
                    <td className="px-3 py-2 text-right text-slate-500 dark:text-slate-400">{money(p.savings)}</td>
  ...
  return (
    <Shell title={`Share-out #${id}`} onClose={onClose}>
      {!data ? <p className="text-sm text-slate-500 dark:text-slate-400">Loading…</p> : (
        <div>
          <p className="text-sm text-slate-600 dark:text-slate-400 mb-3">{money(data.total_amount)} · {data.basis} · {data.member_count} members</p>
          <div className="border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-xs uppercase"><tr><th className="text-left px-3 py-2">Member</th><th className="text-right px-3 py-2">Share</th></tr></thead>
              <tbody>
                {data.shares.map((s, i) => (
                  <tr key={i} className="border-t border-slate-100 dark:border-slate-700">
                    <td className="px-3 py-2 text-slate-800 dark:text-slate-100">{s.first_name} {s.last_name} <span className="text-slate-400 dark:text-slate-400 font-mono text-xs">{s.member_no}</span></td>
...
function Shell({ title, onClose, children, wide }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className={`bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full ${wide ? "max-w-xl" : "max-w-md"} my-10`} onClick={(e) => e.stopPropagation()}>

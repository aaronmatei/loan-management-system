    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-green-100 mb-6 overflow-hidden">
      ...
      <div className="p-5">
        {loading ? (
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">No M-Pesa activity yet. STK requests appear here.</p>
        ) : (
              ...
              <tbody>
                {rows.map((t) => (
                  <tr key={t.id} className="border-t border-slate-100 dark:border-slate-700">
                    ...
                    <td className="px-3 py-2 text-slate-800 dark:text-slate-100">{t.first_name ? `${t.first_name} ${t.last_name}` : t.phone_number}</td>
                    <td className="px-3 py-2 text-slate-600 dark:text-slate-400">{PURPOSE[t.purpose] || t.purpose}</td>
                    <td className="px-3 py-2 text-right">{money(t.amount)}</td>
                    <td className="px-3 py-2 text-slate-500 dark:text-slate-400 font-mono text-xs">{t.mpesa_receipt_number || "—"}</td>

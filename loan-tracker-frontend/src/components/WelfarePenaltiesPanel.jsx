...
export default function WelfarePenaltiesPanel({ welfareId }) {
  ...
  const tab = (on) => `px-3 py-1 text-sm font-semibold rounded-lg ${on ? "bg-rose-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`;
    ...
    <div className="bg-white rounded-xl shadow-md border border-rose-100 mb-6 overflow-hidden">
      ...
      <div className="p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-slate-500">Fines are set when you create a contribution, event or meeting — and charged automatically. This is the record.</p>
        ...
        {loading ? (
          ...
          <p className="text-sm text-slate-500">{showAll ? "No fines recorded." : "No outstanding fines. 🎉"}</p>
        ) : (
              ...
              <tbody>
                {penalties.map((a) => (
                  <tr key={a.id} className="border-t border-slate-100">
                    <td className="px-4 py-2 text-slate-800 whitespace-nowrap">{a.first_name} {a.last_name}</td>
                    <td className="px-4 py-2">
                      ...
                      <span className="text-slate-600">{a.source_label || "—"}</span>
                    </td>
                    <td className="px-4 py-2 text-slate-600">{TRIGGER_LABEL[a.trigger] || a.trigger}</td>
                    <td className="px-4 py-2 text-right font-semibold">{money(a.amount - a.paid_amount)}{Number(a.paid_amount) > 0 && Number(a.paid_amount) < Number(a.amount) ? <span className="text-xs text-slate-400"> of {money(a.amount)}</span> : null}</td>

...
// Creates a pawn-only account separate from other tenants and drops the owner
// straight into their pawns.
export default function PawnbrokerRegister() {
  ...
  const set = (k) => (e) => {
    ...
    setForm((f) => ({
      ...f,
      [k]: v,
      ...(k === "business_name" && !touchedSub ? { subdomain: slug(v) } : {}),
    }));
  };
  ...
  const submit = async (e) => {
    e.preventDefault();
    setError("");
    if (!form.business_name.trim()) return setError("Pawnshop name is required.");
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to create pawnbroker account.");
      setBusy(false);
    }
  };

  const fld = "w-full px-3 py-2.5 border-2 border-gray-200 rounded-lg focus:border-amber-500 focus:outline-none";
  const lbl = "block text-sm font-semibold text-gray-700 mb-1";

  return (
    <div
      className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-yellow-50 bg-cover bg-center bg-no-repeat flex items-center justify-center p-4"
    ...
    >
      <div className="w-full max-w-lg">
        ...
        <form onSubmit={submit} className="bg-white rounded-2xl shadow-xl p-6 space-y-4">
          ...
          <div>
            <label className={lbl}>Pawnshop name *</label>
            <input value={form.business_name} onChange={set("business_name")} placeholder="QuickCash Pawn" className={fld} />
          </div>
          <div>
          ...
          <p className="text-center text-sm text-gray-600">
            Already registered? <Link to="/pawn/login" className="text-amber-700 font-semibold hover:underline">Log in</Link>
          </p>
        </form>
      </div>
    </div>

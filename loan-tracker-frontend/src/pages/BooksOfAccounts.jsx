...
const Row = ({ label, value, bold, indent, tone }) => (
  <div className={`flex items-center justify-between py-1.5 ${bold ? "font-bold text-slate-900 border-t border-slate-200 mt-1 pt-2" : "text-slate-600"}`}>
    <span className={indent ? "pl-4" : ""}>{label}</span>
    <span className={`tabular-nums ${tone || (bold ? "text-slate-900" : "text-slate-800")}`}>{value}</span>
  </div>
);
...
export default function BooksOfAccounts() {
  ...
  if (loading) return <div className="p-4 lg:p-8 max-w-5xl mx-auto"><div className="bg-white rounded-xl shadow-md p-12"><Spinner centered label="Building the books…" /></div></div>;
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 mb-1"><BookOpen className="text-indigo-600" /> Books of Accounts</h1>
      <p className="text-slate-500 mb-6 text-sm">Derived from your ledgers on a cash basis — every statement reconciles with the Capital position.</p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card title="Income Statement" note="Cash basis">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-1 mb-1">Income</p>
          ...
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-3 mb-1">Expenditure</p>
        ...
        <Card title="Balance Sheet">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-1 mb-1">Assets</p>
          ...
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-3 mb-1">Financed by</p>
        ...
        <Card title="Trial Balance">
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 text-sm">
            ...
            <span className="text-xs font-semibold text-slate-400 uppercase text-right">Debit</span>
            <span className="text-xs font-semibold text-slate-400 uppercase text-right">Credit</span>
            {tb.debits.map((d) => (<React.Fragment key={d.account}><span className="text-slate-600 py-1">{d.account}</span><span className="tabular-nums text-right py-1">{KES(d.amount)}</span><span className="py-1" /></React.Fragment>))}
            {tb.credits.map((c) => (<React.Fragment key={c.account}><span className="text-slate-600 py-1">{c.account}</span><span className="py-1" /><span className="tabular-nums text-right py-1">{KES(c.amount)}</span></React.Fragment>))}
            <span className="font-bold text-slate-900 border-t border-slate-200 pt-2">Totals</span>
            <span className="font-bold text-slate-900 tabular-nums text-right border-t border-slate-200 pt-2">{KES(tb.debit_total)}</span>
            <span className="font-bold text-slate-900 tabular-nums text-right border-t border-slate-200 pt-2">{KES(tb.credit_total)}</span>
          </div>
        </Card>
        ...
        <Card title="Loan Portfolio" note={`Portfolio at risk (PAR): ${pf.par_pct}%`}>
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 text-sm">
            ...
            <span className="text-xs font-semibold text-slate-400 uppercase text-right">Loans</span>
            <span className="text-xs font-semibold text-slate-400 uppercase text-right">Outstanding</span>
            {Object.entries(pf.by_status).map(([s, v]) => (
              <React.Fragment key={s}>
                <span className="text-slate-600 py-1">{STATUS_LABEL[s] || s}</span>
            ...
            <span className="font-bold text-slate-900 border-t border-slate-200 pt-2">Total outstanding</span>
            <span className="border-t border-slate-200 pt-2" />
            <span className="font-bold text-slate-900 tabular-nums text-right border-t border-slate-200 pt-2">{KES(pf.total_outstanding)}</span>
          </div>
        </Card>
      </div>
    </div>

...
// detail row. Recipient (client) is pinned (sticky). Each cell() carries
// the EXACT rendering — including dark variants — from the prior table.
const SMS_COLUMNS = [
  {
    key: "created_at",
    label: "Date",
...
// is always shown outside this set; anything not listed drops into the
...
const COLUMN_PRESETS = {
  ...
  detailed: {
    label: "Detailed",
    keys: ["created_at", "phone_number", "message_type", "message", "status"],
  },
  full: {
    label: "Everything",
    keys: SMS_COLUMNS.map((c) => c.key),
  },
};

const PRESET_STORAGE_KEY = "sms.columnPreset";
const SEGMENTS_STORAGE_KEY = "sms.segments";
...
function SMS() {
  ...
  const handleSaveSegment = () => {
    const name = window.prompt("Name this segment (e.g. Failed reminders)");
    if (!name) return;
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="bg-surface rounded-xl shadow-md overflow-hidden">
        ...
        {/* Desktop — shared DataTable (column presets, expandable rows,
            sticky pinned Client, skeleton, scroll affordance). */}
        <DataTable
          columns={SMS_COLUMNS}
          rows={paginatedLogs}
          rowKey={(log) => log.id}
